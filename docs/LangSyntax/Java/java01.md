...
사실 이 간단한 코드의 모든 구성 요소를 막힘 없이 설명할 수 있으면 Java의 기초는 숙지했다고 할 수 있다. **접근제어자**, **클래스**, **static**, **메소드** 등, Java의 중요한 기본기들을 대부분 포함하고 있는 좋은 예제이다. 기능은 *Hello, world!*라는 문자열을 출력하는게 전부이지만, Java를 알아가는데는 너무 중요한 코드다. 내가 정리한 Java 문법을 모두 공부하고 돌아왔을 때 이 코드를 완벽히 설명할 수 있을 때까지 복습할 것이다.